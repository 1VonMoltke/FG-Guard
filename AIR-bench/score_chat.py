...
# OpenAI-compatible LLM API (e.g. DeepSeek). Read from env so the key is
# never committed; launch with:
...
def run_chat_gen(args):
    out_file = args.out_file or os.path.splitext(args.in_file)[0] + '_result.jsonl'
...
def build_test_file(args):
    ...
    with open(args.in_file, 'w', encoding='utf-8') as writer:
            with open(os.path.join(args.root, args.model_result), 'r') as fp:
                for data in fp:
                    row = json.loads(data)
                    ...
                    tmp_d = {
                        # Stable across retries so a partial output can resume without duplicates.
                        'uuid': str(row['uniq_id']),
                        'openai_args': {
...
if __name__ == '__main__':
    ...
    parser.add_argument("-mr", "--model-result", type=str, default='Chat_result_clean.jsonl',
                        help='Inference output jsonl (from Inference_Chat.py) inside --root')
